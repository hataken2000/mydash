...
async function renderSessions() {
  const sessions = await chrome.runtime.sendMessage({ type: 'WS_GET_SESSIONS' });
  ...
  for (const session of sessions) {
    ...
    item.querySelector('.btn-restore').addEventListener('click', async (e) => {
      ...
      if (target) {
        ...
        await chrome.runtime.sendMessage({ type: 'WS_RESTORE_SESSION', session: target });
        window.close();
      }
    });
    ...
    item.querySelector('.btn-delete').addEventListener('click', async (e) => {
      const id = e.target.dataset.id;
      await chrome.runtime.sendMessage({ type: 'WS_DELETE_SESSION', id });
      await renderSessions();
    });
...
function startRename(el, session) {
  ...
  const commit = async () => {
    ...
    await chrome.runtime.sendMessage({ type: 'WS_RENAME_SESSION', id: session.id, name: newName });
    await renderSessions();
  };
...
document.getElementById('exportBtn').addEventListener('click', async () => {
  ...
  a.href = url;
  a.download = `mydash-sessions-${new Date().toISOString().slice(0, 10)}.json`;
  a.click();
...
document.getElementById('importFile').addEventListener('change', async (e) => {
  ...
  await chrome.runtime.sendMessage({ type: 'WS_IMPORT_SESSIONS', sessions: imported });
  e.target.value = '';
  await renderSessions();
});
